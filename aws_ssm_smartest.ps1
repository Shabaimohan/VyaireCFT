...
ForEach ($Param in $Parameters) {
    ...
    If ($Value.Length -gt 0) {
        ...
        # Determine the tier
        $TierOption = if ($Param[0] -eq "apns-certificate") { "--tier Advanced" } else { "" }

        # Check if parameter exists
        If ($ExistingParam) {
            aws ssm put-parameter --name $ParamName --value $PlainTextValue --type "SecureString" $TierOption --overwrite --region $Region | Out-Null
            Write-Host "  - Updated parameter: $ParamName" -ForegroundColor Green
        } Else {
            aws ssm put-parameter --name $ParamName --description $Prompt --value $PlainTextValue --type "SecureString" $TierOption --region $Region | Out-Null
            Write-Host "  - Created new parameter: $ParamName" -ForegroundColor Green
        }
    }
}
